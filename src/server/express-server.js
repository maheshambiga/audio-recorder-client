...
const setRouteHandler = () =>
  new Promise((resolve, reject) => {
    ...
    const registerRoutes = require(
      webapp(defaultConfig.$('plugins.webapp.module'))); //eslint-disable-line
  ...
  new Promise((resolve, reject) => {
    app.listen(defaultConfig.$('connections.default.port'), err => {
      if (err) {
        ...
        console.log(`App listening on port: ${defaultConfig.$(
...
module.exports = function expressServer(userConfig, callback) {
  const promise = Promise.resolve(userConfig)
  .then(loadConfigs).then(setStaticPaths).then(setRouteHandler).then(startServer);
